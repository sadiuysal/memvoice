...
from zep_python import Memory, SearchType, SearchScope
...
async def test_create_memory(memory_service, mock_db, mock_zep_env):
    ...
    mock_db.refresh = MagicMock(side_effect=lambda obj: setattr(obj, 'created_at', datetime.utcnow()) or setattr(obj, 'updated_at', datetime.utcnow()))
...
async def test_search_memories(memory_service, mock_db, mock_zep_env):
    ...
    mock_memories = [
        MemoryEntry(id=1, user_id="test_user", content="Memory 1", created_at=datetime.utcnow(), updated_at=datetime.utcnow()),
        MemoryEntry(id=2, user_id="test_user", content="Memory 2", created_at=datetime.utcnow(), updated_at=datetime.utcnow())
    ]
